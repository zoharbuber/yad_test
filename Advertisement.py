...
class Advertisment:
    ...
    def scrape_yad2(self):
        ...
        r = requests.get(url, headers={'user-agent': 'Mozilla/2.0 (Macintosh; Intel Mac OS X 11_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.88 Safari/537.36'})
        soup = bs(r.content)
        all_advertisments = soup.find_all("div", {'class': 'feeditem table'})
        for data in all_advertisments:
